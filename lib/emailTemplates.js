...
export function notificationEmail(lead) {
  ...
  const slotDate = prettyDate(lead.preferredDate);
  ...
  const bodyHtml = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 6px 0;border:1px solid ${LINE};border-radius:10px;background-color:${TINT};">
      <tr>
        <td style="padding:18px 20px;">
          <p style="margin:0 0 6px 0;font-family:${FONT};font-size:11px;font-weight:bold;letter-spacing:1.2px;text-transform:uppercase;color:${VIOLET};">Requested slot</p>
          <p style="margin:0 0 3px 0;font-family:${FONT};font-size:18px;line-height:25px;font-weight:bold;color:${HEADING};">${esc(slotDate)}</p>
          <p style="margin:0;font-family:${FONT};font-size:14px;line-height:20px;color:${BODY};">${esc(lead.preferredTime)} &middot; ${esc(lead.timezone)}</p>
        </td>
      </tr>
    </table>
  ...
  const text = [
    ...
    ``,
    `REQUESTED SLOT`,
    `${slotDate} at ${lead.preferredTime} (${lead.timezone})`,
    ``,
    `CONTACT`,
    `Name:       ${name}`,
  ...
  return {
    ...
    html: shell({
      preheader: `${lead.company} · ${slotDate} at ${lead.preferredTime}`,
...
export function confirmationEmail(lead) {
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 22px 0;font-family:${FONT};font-size:15px;line-height:25px;color:${BODY};">
      ...
      your details, and the slot you requested is below.
    </p>

    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px 0;border:1px solid ${LINE};border-radius:10px;background-color:${TINT};">
      <tr>
        <td style="padding:18px 20px;">
          <p style="margin:0 0 6px 0;font-family:${FONT};font-size:11px;font-weight:bold;letter-spacing:1.2px;text-transform:uppercase;color:${VIOLET};">Your requested slot</p>
          <p style="margin:0 0 3px 0;font-family:${FONT};font-size:18px;line-height:25px;font-weight:bold;color:${HEADING};">${esc(slotDate)}</p>
          <p style="margin:0;font-family:${FONT};font-size:14px;line-height:20px;color:${BODY};">${esc(lead.preferredTime)} &middot; ${esc(lead.timezone)}</p>
        </td>
      </tr>
    ...
    <p style="margin:0 0 16px 0;font-family:${FONT};font-size:15px;line-height:25px;color:${BODY};">
      Roney Solomon will confirm this slot by email within one business day. If
      the time no longer suits you, just reply to this message and we will
      rearrange it.
    </p>
...
Thank you for asking for a conversation about Frugal Innovation. We have your details, and the slot you requested is below.

YOUR REQUESTED SLOT
${slotDate} at ${lead.preferredTime} (${lead.timezone})

Roney Solomon will confirm this slot by email within one business day. If the time no longer suits you, just reply to this message and we will rearrange it.
...
www.ignitho.com`;
  ...
  return {
    ...
    html: shell({
      preheader: `${slotDate} at ${lead.preferredTime} — we will confirm within one business day.`,
